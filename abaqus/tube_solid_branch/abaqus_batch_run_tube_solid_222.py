# -*- coding: utf-8 -*-
"""
Abaqus batch runner for CurviStrut-Lattice tube-solid branch (222).

Run from project root:
    abaqus cae noGUI=abaqus/tube_solid_branch/abaqus_batch_run_tube_solid_222.py

This script:
    1) scans data/rep222/json/*_rep222.json
...
THIS_DIR = get_script_dir('abaqus_batch_run_tube_solid_222.py')
...
# ---------------- USER SETTINGS ----------------
RUN_ROOT = os.path.join(PROJECT_ROOT, 'runs', 'tube_solid_222')
JSON_DIR = os.path.join(PROJECT_ROOT, 'data', 'rep222', 'json')
...
BRANCH_NAME = '2x2x2'
JSON_PATTERN = '*_rep222.json'
SUMMARY_FILE = 'tube_solid_222_batch_status.csv'
...
MIN_INC = 1e-8
MAX_NUM_INC = 1000
CONTOUR_FREQUENCY = 'LAST_INCREMENT'  # use 1 only for full stress animation

# Tube mesh resolution. Higher values look smoother but increase cost.
RING_SEGMENTS = 8
AXIAL_STEP = 3       # use every 3rd centerline point; set 2 or 1 for smoother geometry

NUM_CPUS = 8
NUM_DOMAINS = 8
MEMORY_PERCENT = 90
MAX_CASES = 1        # set None after 222 validation
...
def main():
    ...
    print('CurviStrut tube-solid batch (222)')
    ...
    if len(files) == 0:
        raise RuntimeError('No *_rep222.json files found. Run batch_repeat222_centerline first.')
